    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-amber-50 border border-amber-100 rounded-lg p-4">
          ...
          <h3 className="text-2xl font-semibold mt-1">{upcomingReservations.length}</h3>
          <p className="text-sm text-amber-800 mt-2">Next - {upcomingReservations[0]?.restaurant} on {upcomingReservations[0]?.date}</p>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
          ...
          <h3 className="text-2xl font-semibold mt-1">{favoriteRestaurants.length}</h3>
          <p className="text-sm text-blue-800 mt-2">Most visited - {favoriteRestaurants[0]?.name}</p>
        </div>
        ...
        <div className="bg-green-50 border border-green-100 rounded-lg p-4">
          ...
          <p className="text-sm text-green-800 mt-2">Expiring soon - {activeOffers[0]?.title}</p>
        </div>
      </div>
            ...
            {upcomingReservations.length > 0 ? (
              <div className="space-y-4">
                {upcomingReservations.map((reservation) => (
                  <div key={reservation.id} className="p-3 bg-stone-50 border-none rounded-lg transition-colors">
                    <div className="flex justify-between">
                      <h4 className="font-medium text-stone-800">{reservation.restaurant}</h4>
            ...
            {favoriteRestaurants.length > 0 ? (
              <div className="space-y-4">
                {favoriteRestaurants.map((restaurant) => (
                  <div key={restaurant.id} className="p-3 bg-stone-50 border-none rounded-lg transition-colors">
                    <div className="flex justify-between">
                      <h4 className="font-medium text-stone-800">{restaurant.name}</h4>
                      <div className="flex items-center text-stone-600">
                        <Star size={14} weight="fill" className="text-amber-400 mr-1" />
        ...
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {activeOffers.map((offer) => (
              <div key={offer.id} className="p-4 border border-amber-100 bg-amber-50 rounded-lg">
                <div className="flex justify-between">
                  <h4 className="font-medium text-amber-800">{offer.title}</h4>
                  <p className="text-xs text-amber-600">Expires: {offer.expires}</p>
                </div>
                <p className="text-sm text-amber-700 mt-1">At: {offer.restaurant}</p>
                <div className="mt-3 p-2 bg-amber-100 border border-dashed border-amber-400 rounded-full text-center">
                  <span className="text-amber-800 font-mono text-sm tracking-wider">{offer.code}</span>
                </div>
              </div>
            ))}
          </div>
