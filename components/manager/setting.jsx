...
import { GearSix, Bell, LockKey, Eye, Clock, Globe, Sun, Moon } from "@phosphor-icons/react";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-medium text-stone-800">Notification Preferences</h3>
          </div>
          
          <div className="space-y-4">
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Notification Channels</p>
              <div className="space-y-2">
                {Object.keys(notifications).slice(0, 3).map((key) => (
                  <label key={key} className="flex items-center gap-3 cursor-pointer">
                    <div className="relative flex items-center">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className={`w-10 h-6 transition rounded-full ${notifications[key] ? 'bg-stone-800' : 'bg-stone-300'}`}></div>
                      <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition transform ${notifications[key] ? 'translate-x-4' : ''}`}></div>
                    </div>
                    <span className="text-stone-700 text-sm capitalize">{key} Notifications</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Alert Types</p>
              <div className="space-y-2">
                {Object.keys(notifications).slice(3, 6).map((key) => (
                  <label key={key} className="flex items-center gap-3 cursor-pointer">
                    <div className="relative flex items-center">
                      <input
                        type="checkbox"
                    ...
                    <span className="text-stone-700 text-sm capitalize">{key} Alerts</span>
                  </label>
                ))}
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Password</p>
            ...
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Two-Factor Authentication</p>
              <label className="flex items-center gap-3 cursor-pointer">
                <div className="relative flex items-center">
                  <input
                    ...
                    className="sr-only"
                  />
                  <div className={`w-10 h-6 transition rounded-full bg-stone-800`}></div>
            ...
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Session Management</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-medium text-stone-800">Display & Language</h3>
          </div>
          
          <div className="space-y-4">
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Appearance</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Time Format</p>
              <div className="flex items-center gap-3">
                <button 
            ...
            <div>
              <p className="text-sm font-medium text-stone-700 mb-2">Time Zone</p>
              <select 
                className="w-full p-2 border border-stone-300 rounded-lg focus:outline-none focus:ring-stone-500 focus:border-stone-500"
              >
