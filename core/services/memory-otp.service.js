...
exports.sendOTPviaSMS = async (phoneNumber, otpCode) => {
  ...
  console.log(`📱 [MEMORY] MOCK SMS to ${phoneNumber}: Your verification code is ${otpCode}`);
  return true;
};
...
exports.verifyOTP = async (phoneNumber, otpCode) => {
  console.log(`🔄 [MEMORY] verifyOTP called for phone: ${phoneNumber}, otp: ${otpCode}`);
  ...
  try {
    ...
    console.log(`✅ [MEMORY] OTP record found for ${phoneNumber}: ${record.otp}`);
    
    // Check if OTP matches
    if (record.otp !== otpCode) {
      console.log(`❌ [MEMORY] OTP mismatch: expected ${record.otp}, got ${otpCode}`);
