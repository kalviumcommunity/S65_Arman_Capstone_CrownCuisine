...
console.log(`🔧 Twilio client initialized: ${twilioClient ? 'Yes' : 'No (Development Mode)'}`);
...
exports.generateOTP = async (phoneNumber) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`💾 New OTP created in database: ${JSON.stringify({
        id: newOtp._id,
        phoneNumber: newOtp.phoneNumber,
        createdAt: newOtp.createdAt
      })}`);
    } catch (createError) {
      console.error(`❌ Error creating OTP in database: ${createError.message}`);
      throw createError;
    }
...
exports.sendOTPviaSMS = async (phoneNumber, otpCode) => {
  ...
  try {
    ...
    if (!config.twilioPhoneNumber) {
      console.log(`[DEV MODE] Using development flow since no Twilio phone number is configured`);
...
exports.verifyOTP = async (phoneNumber, otpCode) => {
  ...
  try {
    ...
    console.log(`✅ OTP record found: ${JSON.stringify({
      id: otpRecord._id,
